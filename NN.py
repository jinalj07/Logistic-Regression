...
'''
    ...
    In this problem, we will implement a classification method using fully-connected neural network (FC) with two layers.
    ...
    We will re-use the functions you implemented in Softmax Regression. 
    For example, sr.softmax() represents the softmax function you implemented in Softmax Regression.
...
def compute_z1(x,W1,b1):
    '''
        Computing the linear logit values of a data instance in the first layer. z1 =  W1 x + b1
        Input:
...
def compute_a1(z1):
    '''
        Computing the sigmoid activations a1 from the linear logits z1 in the first layer. 
        Input:
...
def compute_a2(z2):
    '''
        Computing the softmax activations a2 from the linear logits z2 in the second layer. 
        Input:
    ...
    a2 = np.exp(Temp)/(sum(np.exp(Temp)))
...
def forward(x, W1, b1, W2, b2):
    '''
       Forward pass: given an instance in the training data, computing the logits z, activations a in each layer. 
        Input:
            x: the feature vector of a data instance, a float numpy vector of shape p by 1. 
...
def compute_dL_da2(a2, y):
    '''
        Computing local gradient of the multi-class cross-entropy loss function L w.r.t. the activations a2 in the 2nd layer.
        Input:
    ...
    return np.matrix(dL_da2)

#-----------------------------------------------------------------
def compute_da2_dz2(a2):
    '''
        Computing local gradient of the softmax activations a2 w.r.t. the logits z2 in the 2nd layer.
        Input:
    ...
    return np.matrix(da2_dz2) 


#-----------------------------------------------------------------
def compute_dz2_dW2(a1,c):
    '''
        Computing local gradient of the logits function z2 w.r.t. the weights W2. 
        Input:
...
def compute_dz2_db2(c):
    '''
        Computing local gradient of the logits function z2 w.r.t. the biases b2. 
        Input:
...
def compute_dz2_da1(W2):
    '''
        Computing local gradient of the logits z2 w.r.t. the activations a1.
        Input:
...
def compute_da1_dz1(a1):
    '''
        Computing local gradient of the sigmoid activations a1 w.r.t. the logits z1 in the first layer.
        Input:
            a1: the activations of sigmoid function, a numpy float vector of shape h by 1. 
            a1: the non-linear activations in the 1st layer.
        Output:
...
def compute_dz1_dW1(x,h):
    '''
        Computing local gradient of the logits function z1 w.r.t. the weights W1 in the 1st layer. 
        Input:
            x: the feature vector of a data instance, a float numpy vector of shape p by 1. Here p is the number of features/dimensions.
...
def compute_dL_da1(dL_dz2,dz2_da1):
    '''
        Computing local gradient of the loss function L w.r.t. the activations a1 using chain rule.
        Input:
    ...
    dz2_da1_tran = np.transpose(dz2_da1)
    dL_da1 = (dz2_da1_tran*dL_dz2)
...
def compute_dL_dz1(dL_da1,da1_dz1):
    '''
        Computing local gradient of the loss function L w.r.t. the logits z1 using chain rule.
...
def predict(Xtest, W1,b1,W2,b2):
    '''
       Predicting the labels of the instances in a test dataset using fully connected network.
...
def check_da1_dz1(z1,delta= 1e-7):
    '''
        Computing local gradient of the sigmoid activations a using gradient check.
        Input:
...
def check_dL_dW2(x,y, W1,b1,W2,b2, delta= 1e-7):
    '''
        Computing gradient of the weights W1 a using gradient check.
        Input:
            delta: a small number for gradient check, a float scalar.
        Output:
