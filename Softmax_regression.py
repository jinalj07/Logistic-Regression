...
'''
    ...
    In this problem, we will implement the softmax regression for multi-class classification problems.
...
def compute_z(x,W,b):
    '''
        Computing the linear logit values of a data instance. z =  W x + b
        Input:
...
def compute_a(z):
    '''
        Computing the softmax activations.
        Input:
...
def compute_L(a,y):
    '''
        Computing multi-class cross entropy, which is the loss function of softmax regression. 
        Input:
...
def forward(x,y,W,b):
    '''
       Forward pass: given an instance in the training data, computing the logits z, activations a and multi-class cross entropy L on the instance.
        Input:
...
def compute_dL_da(a, y):
    '''
        Computing local gradient of the multi-class cross-entropy loss function w.r.t. the activations.
        Input:
            a: the activations of a training instance, a float numpy vector of shape c by 1. Here c is the number of classes. 
            y: the label of a training instance, an integer scalar value. The values can be 0,1,2, ..., or (c-1).
        Output:
    ...
    return np.matrix(dL_da) 


#-----------------------------------------------------------------
def compute_da_dz(a):
    '''
        Computing local gradient of the softmax activations a w.r.t. the logits z.
        Input:
    ...
    return np.matrix(da_dz) 


#-----------------------------------------------------------------
def compute_dz_dW(x,c):
    '''
        Computing local gradient of the logits function z w.r.t. the weights W.
        Input:
...
def compute_dz_db(c):
    '''
        Computing local gradient of the logits function z w.r.t. the biases b. 
        Input:
            c: the number of classes, an integer. 
        Output:
...
def backward(x,y,a):
    '''
       Back Propagation: given an instance in the training data, computing the local gradients of the logits z, activations a, weights W and biases b on the instance. 
        Input:
            x: the feature vector of a training instance, a float numpy vector of shape p by 1. Here p is the number of features/dimensions.
            y: the label of a training instance, an integer scalar value. The values can be 0,1,2, ..., or (c-1).
            a: the activations of a training instance, a float numpy vector of shape c by 1. Here c is the number of classes. 
...
def compute_dL_dz(dL_da,da_dz):
    '''
       Given the local gradients, computing the gradient of the loss function L w.r.t. the logits z using chain rule.
        Input:
            dL_da: the local gradients of the loss function w.r.t. the activations, a float numpy vector of shape c by 1. 
                   The i-th element dL_da[i] represents the partial gradient of the loss function L w.r.t. the i-th activation a[i]:  d_L / d_a[i].
            da_dz: the local gradient of the activation w.r.t. the logits z, a float numpy matrix of shape (c by c). 
...
def compute_dL_dW(dL_dz,dz_dW):
    '''
       Given the local gradients, computing the gradient of the loss function L w.r.t. the weights W using chain rule. 
        Input:
            dL_dz: the gradient of the loss function L w.r.t. the logits z, a numpy float vector of shape c by 1. 
                   The i-th element dL_dz[i] represents the partial gradient of the loss function L w.r.t. the i-th logit z[i]:  d_L / d_z[i].
            dz_dW: the partial gradient of logits z w.r.t. the weight matrix W, a numpy float matrix of shape (c by p). 
...
def compute_dL_db(dL_dz,dz_db):
    '''
       Given the local gradients, computing the gradient of the loss function L w.r.t. the biases b using chain rule.
        Input:
            dL_dz: the gradient of the loss function L w.r.t. the logits z, a numpy float vector of shape c by 1. 
                   The i-th element dL_dz[i] represents the partial gradient of the loss function L w.r.t. the i-th logit z[i]:  d_L / d_z[i].
...
def update_W(W, dL_dW, alpha=0.001):
    '''
       Updating the weights W using gradient descent.
        Input:
...
def update_b(b, dL_db, alpha=0.001):
    '''
       Updating the biases b using gradient descent.
        Input:
...
def train(X, Y, alpha=0.01, n_epoch=100):
    '''
       Given a training dataset, training the softmax regression model by iteratively updating the weights W and biases b using the gradients computed over each data instance. 
        Input:
...
def predict(Xtest, W, b):
    '''
       Predicting the labels of the instances in a test dataset using softmax regression.
        Input:
...
def check_da_dz(z, delta=1e-7):
    '''
        Computing local gradient of the softmax function using gradient checking.
        Input:
            z: the logit values of softmax regression, a float numpy vector of shape c by 1. Here c is the number of classes
...
def check_dL_da(a, y, delta=1e-7):
    '''
        Computing local gradient of the multi-class cross-entropy function w.r.t. the activations using gradient checking.
        Input:
            a: the activations of a training instance, a float numpy vector of shape c by 1. Here c is the number of classes. 
            y: the label of a training instance, an integer scalar value. The values can be 0,1,2, ..., or (c-1).
            delta: a small number for gradient check, a float scalar.
...
def check_dz_dW(x, W, b, delta=1e-7):
    '''
        computing the local gradient of the logit function using gradient check.
        Input:
            x: the feature vector of a data instance, a float numpy vector of shape p by 1. Here p is the number of features/dimensions.
            W: the weight matrix of softmax regression, a float numpy matrix of shape (c by p). Here c is the number of classes.
            b: the bias values of softmax regression, a float numpy vector of shape c by 1.
...
def check_dL_dW(x,y,W,b,delta=1e-7):
    '''
       Computing the gradient of the loss function w.r.t. the weights W using gradient checking.
        Input:
            x: the feature vector of a training instance, a float numpy vector of shape p by 1. Here p is the number of features/dimensions.
            y: the label of a training instance, an integer scalar value. The values can be 0,1,2, ..., or (c-1).
            W: the weight matrix of softmax regression, a float numpy matrix of shape (c by p). Here c is the number of classes.
...
def check_dL_db(x,y,W,b,delta=1e-7):
    '''
       Computing the gradient of the loss function w.r.t. the bias b using gradient checking.
        Input:
            x: the feature vector of a training instance, a float numpy vector of shape p by 1. Here p is the number of features/dimensions.
            y: the label of a training instance, an integer scalar value. The values can be 0,1,2, ..., or (c-1).
            W: the weight matrix of softmax regression, a float numpy matrix of shape (c by p). Here c is the number of classes.
